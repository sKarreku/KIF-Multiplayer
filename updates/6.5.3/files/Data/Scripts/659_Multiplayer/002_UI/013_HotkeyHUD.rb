    ...
    class HotkeyHUD
      ...
      def update
        ...
        begin
          ...
          state = [chat_on, in_squad, profile_open]
          if @last_state != state
            @last_state = state
      ...
      # Teal/cyan color: Color.new(50, 170, 170)
      ...
      def _briefcase_fill(bmp, ix, iy, c)
        # Handle arch at top center (3 wide, 3 tall)
      ...
      def _id_card_fill(bmp, ix, iy, c, em)
        # Card rectangle
        bmp.fill_rect(ix + 1, iy + 2,  20, 18, c)   # card body
        bmp.fill_rect(ix + 2, iy + 1,  18,  1, c)   # top rounded
        bmp.fill_rect(ix + 2, iy + 20, 18,  1, c)   # bottom rounded
        # Portrait circle (left side of card)
        bmp.fill_rect(ix + 3, iy + 4,  5, 1, em)
        bmp.fill_rect(ix + 2, iy + 5,  7, 5, em)
        bmp.fill_rect(ix + 3, iy + 10, 5, 1, em)
        # Info lines (right side of card)
        bmp.fill_rect(ix + 11, iy + 6,  8, 2, em)   # name line
        bmp.fill_rect(ix + 11, iy + 10, 6, 1, em)   # sub line 1
        bmp.fill_rect(ix + 11, iy + 13, 6, 1, em)   # sub line 2
        bmp.fill_rect(ix + 11, iy + 16, 4, 1, em)   # sub line 3
      end
    end
  end
