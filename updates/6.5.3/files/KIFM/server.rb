...
egg_hatched_timestamps    = {}  # { uuid => [timestamps] } — max 30/hour
...
def profile_stats_increment!(stats, uuid, stat, amount = 1)
  ...
  PROFILE_STATS_MUTEX.synchronize do
    stats[uuid] ||= { "wild_fainted"=>0, "wild_captured"=>0, "trainer_battles"=>0, "badges"=>0, "eggs_hatched"=>0, "last_updated"=>0 }
        ...
        if data
          ...
          if data == "STAT_EGG_HATCHED"
            begin
              ...
              if uuid
                now = Time.now.to_i
                egg_hatched_timestamps[uuid] ||= []
                egg_hatched_timestamps[uuid].reject! { |ts| now - ts > 3600 }
                if egg_hatched_timestamps[uuid].length < 30
                  egg_hatched_timestamps[uuid] << now
                  profile_stats_increment!(profile_stats, uuid, "eggs_hatched")
                  profile_stats_save!(profile_stats)
                  MultiplayerDebug.info("PROFILE-STAT", "#{sid} eggs_hatched +1")
                else
                  MultiplayerDebug.info("PROFILE-STAT", "#{sid} STAT_EGG_HATCHED rate-limited")
                end
              end
            rescue => e
          ...
          if data.start_with?("REQ_PROFILE:")
            begin
              ...
              stats_data = profile_stats[target_uuid] || {
                "wild_fainted"=>0,"wild_captured"=>0,"trainer_battles"=>0,"badges"=>0,"eggs_hatched"=>0,"last_updated"=>0
              }
          ...
          if disconnect_uuid
            ...
            egg_hatched_timestamps.delete(disconnect_uuid)
