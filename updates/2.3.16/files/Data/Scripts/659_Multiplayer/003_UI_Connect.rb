...
module MultiplayerUI
  ...
  def self.load_settings
    ...
    if File.exist?(SETTINGS_FILE)
      begin
        data = File.read(SETTINGS_FILE).strip.split(":")
  ...
  def self.openCouchCoopMenu
    ...
    commands = ["Enable Keyboard Only", "Enable Controller Only", "Disable", "Cancel"]
    ...
    when 1
      # Enable Controller Only
      CouchCoopConfig.enable("controller")
      pbMessage(_INTL("Couch Co-op enabled. This client will only accept controller input, even when unfocused."))
      ##MultiplayerDebug.info("UI-COOP", "Couch co-op enabled: controller only")
    when 2
      # Disable
