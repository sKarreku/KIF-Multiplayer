...
module PokeBattle_BattleCommon
  ...
  def pbStorePokemon(pkmn)
    ...
    begin
      ...
      ##MultiplayerDebug.info("🔍 STORE-CHECK", "Checking #{pkmn.name}: catcher_sid=#{catcher_sid}, my_sid=#{my_sid}")
      ...
      if !catcher_sid
        ##MultiplayerDebug.warn("⚠️ STORE-NOSID", "No @coop_catcher_sid on #{pkmn.name}, skipping storage")
        return
      end

      # Store to $Trainer (the real player), not trainer objects
      ##MultiplayerDebug.info("💾 STORE-MINE", "Storing #{pkmn.name} to my party ($Trainer)")
    ...
    if my_sid && @caughtPokemon && @caughtPokemon.length > 0
      ...
      my_caught = @caughtPokemon.select do |pkmn|
        ...
        if catcher_sid
          ...
          if is_mine
            ##MultiplayerDebug.info("✅ RECORD-MINE", "  #{pkmn.name}: catcher_sid=#{catcher_sid} matches my_sid=#{my_sid}")
          else
            ##MultiplayerDebug.info("❌ RECORD-NOTMINE", "  #{pkmn.name}: catcher_sid=#{catcher_sid} ≠ my_sid=#{my_sid}")
          end
          is_mine
        else
          # No SID set - this shouldn't happen, assume not mine
          ##MultiplayerDebug.warn("⚠️ RECORD-NOSID", "  #{pkmn.name}: no @coop_catcher_sid set, assuming not mine")
          false
        end
      end

      ##MultiplayerDebug.info("📊 RECORD-FILTER", "Filtered: #{@caughtPokemon.length} total → #{my_caught.length} mine")
