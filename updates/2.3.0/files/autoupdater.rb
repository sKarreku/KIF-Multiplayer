...
def extract_archive(archive_path, dest_dir)
  ...
  command = "rar x -o+ \"#{archive_path}\" \"#{dest_dir}\\\" >nul 2>&1"
...
def read_local_version
  if !File.exist?(LOCAL_VERSION_FILE)
    raise "Local version file not found: #{LOCAL_VERSION_FILE}"
  end
...
def install_update(staged_target, backup_path)
  ...
  begin
    ...
    $logger.log("  Moving new files...")
    FileUtils.mv(staged_target, TARGET_DIR)

    # Verify installation
    if !Dir.exist?(TARGET_DIR)
      raise "Installation failed: target directory not created"
    end

    file_count = Dir.glob(File.join(TARGET_DIR, "**", "*")).select { |f| File.file?(f) }.count
    $logger.log("  Installation complete: #{file_count} files installed")
