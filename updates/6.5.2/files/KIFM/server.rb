  ...
  Thread.new do
    begin
      ...
      http_server = WEBrick::HTTPServer.new(
        Port:            DISCORD_AUTH_PORT,
        BindAddress:     actual_bind,
        Logger:          WEBrick::Log.new(File.open(File::NULL, 'w')),
        AccessLog:       []
      )

      # Step 1 — redirect player to Discord
      http_server.mount_proc('/auth/discord') do |_req, res|
      ...
      http_server.mount_proc('/auth/discord/callback') do |req, res|
        code = req.query['code'].to_s.strip
        if code.empty?
          res.status = 400
          res.body   = "<h2>Missing code — please try again from the game.</h2>"
          next
        end
        ...
        begin
          ...
          res.status = 500
          res.body   = "<h2>Discord error</h2><p>#{e.message}</p><p>Please try again.</p>"
          next
        end

        if discord_id.empty?
          res.status = 400
          res.body   = "<h2>Could not read your Discord ID.</h2><p>Please try again.</p>"
          next
        end
      ...
      at_exit { http_server.shutdown rescue nil }
      http_server.start
    rescue => e
