...
class PokemonSystem
  ...
  attr_accessor :mp_hud_icon_scale           # HUD icon scale percentage (50-200, default 100)
...
class MultiplayerOptScene < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    ...
    # Overworld zoom toggle (temporarily disabled)
    $PokemonSystem.mp_overworld_zoom = 0  # Force off
    options << EnumOption.new(_INTL("Map Zoom"),
      [_INTL("Disabled")],
      proc { 0 },
      proc { |value| },
      ["Map zoom is temporarily disabled"]
    )
    ...
    options << SliderOption.new(_INTL("HUD Icon Size"),
      ...
      proc { |value|
        $PokemonSystem.mp_hud_icon_scale = value
      },
      "Scale overworld HUD icons (Player List, GTS, Cases, etc.) — 50% to 200%"
    )
