        ...
        if data
          ...
          if data.start_with?("BOSS_DEFEATED:")
            begin
              ...
              # Increment bosses_fainted stat for each participant
              participants.each do |psid|
                ps = sid_sockets[psid]
                p_uuid = client_data[ps][:platinum_uuid] rescue nil
                profile_stats_increment!(profile_stats, p_uuid, "bosses_fainted", profile_titles: profile_titles, client_data: client_data, clients: clients, platinum_accounts: platinum_accounts) if p_uuid
              end
