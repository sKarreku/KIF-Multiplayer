...
module BossConfig
  ...
  # Core Settings
  #=============================================================================
  SPAWN_CHANCE = 75           # 1 in X wild encounters becomes a boss (set to 2 for 50/50 testing)
  ...
  LOOT = {
    ...
    rare:      { pool: [:PPMAX, :SHINY_EGG, :MEGARING, :MEGABRACELET, :MEGACUFF, :MEGACHARM], qty: 1, weight: 15 },
    epic:      { pool: :MEGA_STONES_POOL, qty: 1, weight: 8 },
    legendary: { pool: [:MASTERBALL, :SHINY_LEGENDARY_EGG, :RESONANCECORE] + BloodboundCatalysts::ALL_ITEMS, qty: 1, weight: 2 }
  }

  # ── Dynamic epic pool (mega stones + battle items) ──────────────────────
  EPIC_BASE_ITEMS = %i[LEFTOVERS LIFEORB CHOICEBAND CHOICESPECS ASSAULTVEST FOCUSSASH ROCKYHELMET EXPERTBELT WEAKNESSPOLICY AIRBALLOON RESONANCERESONATOR]

  # Lazily builds the epic pool: base battle items + all mega stones
  def self.epic_pool
    @epic_pool ||= begin
      stones = []
      ...
      EPIC_BASE_ITEMS + stones
    end
  end

  def self.reset_epic_pool; @epic_pool = nil; end
  ...
  # Resolve a pool reference (handles :MEGA_STONES_POOL dynamic lookup)
  def self.resolve_pool(pool_ref)
    if pool_ref == :MEGA_STONES_POOL
      epic_pool
    elsif pool_ref.is_a?(Array)
      pool_ref
    else
      [pool_ref]
    end
  ...
  def self.weighted_loot_pick
    ...
    LOOT.each do |rarity, data|
      ...
      if roll < cumulative
        if data[:pool]
          resolved = resolve_pool(data[:pool])
