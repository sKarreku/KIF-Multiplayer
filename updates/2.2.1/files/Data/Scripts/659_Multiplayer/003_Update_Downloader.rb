...
module UpdateDownloader
  ...
  def count_changed_files(files)
    ...
    files.each do |file_info|
      ...
      if !File.exist?(local_path)
        ...
        actual_sha256 = HTTPWrapper.calculate_sha256(local_path)
        changed_count += 1 if actual_sha256 != expected_sha256
      end
    end
  ...
  def download_delta_update(manifest, version)
    ...
    files.each_with_index do |file_info, index|
      ...
      success = HTTPWrapper.http_download_file(file_url, staging_path, expected_sha256)
  ...
  def download_full_archive(manifest, version)
    ...
    return false unless archive_info
    ...
    success = HTTPWrapper.http_download_file(archive_url, local_path, expected_sha256)
