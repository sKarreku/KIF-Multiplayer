    ...
    begin
      ...
      when /^\/give_title\s+(SID\S+)\s+(\S+)$/i
        ...
        if profile_titles[uuid]["owned"].include?(title_id)
          ...
          # Push updated titles to the player
          owned_full = profile_titles[uuid]["owned"].map { |tid| title_to_hash(tid) }.compact
          safe_send(sock, "OWN_TITLES:#{MiniJSON.dump(owned_full)}")
        end
      ...
      when "/titles"
        ...
        TITLE_DEFINITIONS.each { |tid, td| puts "  #{tid.ljust(24)} — #{td["name"]}" }
      ...
      when "/help"
        ...
        puts "  /titles                       — List all available title IDs"
