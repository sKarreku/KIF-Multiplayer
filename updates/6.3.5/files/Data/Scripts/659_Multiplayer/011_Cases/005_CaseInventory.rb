...
module KIFCaseInventory
  ...
  def self.request_sync
    return unless $multiplayer&.connected?
    $multiplayer.send_message("CASE_INV_REQ")
  end
  ...
  def self.buy(case_type)
    ...
    $multiplayer.send_message("CASE_BUY:#{case_type}")
  ...
  def self.open_from_inventory(case_type)
    ...
    $multiplayer.send_message("CASE_OPEN_INV:#{case_type}")
  ...
  def self.buy_and_open(case_type)
    return false unless $multiplayer&.connected?
    $PokemonGlobal.case_result = nil
    $multiplayer.send_message("CASE_BUYOPEN:#{case_type}")
