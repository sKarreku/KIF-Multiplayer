...
class MultiplayerOptScene < PokemonOption_Scene
  ...
  def pbGetOptions(inloadscreen = false)
    ...
    options << EnumOption.new(_INTL("Map Zoom"),
      ...
      proc { $PokemonSystem.mp_overworld_zoom || 0 },
      proc { |value| $PokemonSystem.mp_overworld_zoom = value },
      ["No zoom — normal overworld view",
       "Zoom out 25% — see more of the map (tiles appear smaller)",
       "Zoom out 50% — see a lot more of the map (tiles appear much smaller)"]
    )
