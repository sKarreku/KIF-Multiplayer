...
module MultiplayerUI
  ...
  def self.openPlayerList
    ...
    begin
      ...
      # --- Scroll window (display names only; raw entries kept for parsing) ---
      cmds = players.map do |p|
        _sid, pname, _uuid = MultiplayerUI.parse_player_entry(p)
        pname.to_s.empty? ? p.to_s : pname.to_s
      end
      ...
      loop do
        ...
        if Input.trigger?(Input::BACK)
          ...
          # chosen_line is the display name only — look up the raw entry by index
          raw_entry = players[sel]
          sid, name, uuid = MultiplayerUI.parse_player_entry(raw_entry)
          ...
          # Build per-player actions (View Profile always available if UUID known)
          has_profile = uuid && !uuid.empty?
          actions = [_INTL("View Profile"), _INTL("Invite to Squad"), _INTL("Let's Battle!"), _INTL("Request Trade"), _INTL("Cancel")]
          act = pbMessage(_INTL("Player: {1}", name), actions, 4)

          case act
          when 0
            # View Profile
            if has_profile && defined?(MultiplayerUI::ProfilePanel)
              ...
              pbMessage(_INTL("Profile not available (UUID unknown)."))
            end
