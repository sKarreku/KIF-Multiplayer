...
class PokeBattle_Battle
  ...
  def pbSwitchInBetween(idxBattler, canCancel = false, canChooseCurrent = false)
    ...
    if owner.is_a?(NPCTrainer)
      owner_sid = owner.id
      is_remote_player = (owner_sid != my_sid)
    end
    ...
    # CRITICAL FIX: Don't sync for enemy trainers (indices 1, 3, 5)
    # They're controlled by AI, not networked players
    is_enemy = opposes?(idxBattler)
    if is_enemy
      ##MultiplayerDebug.info("COOP-SWITCH-PHASE", "  ENEMY battler - using original behavior (no sync)")
