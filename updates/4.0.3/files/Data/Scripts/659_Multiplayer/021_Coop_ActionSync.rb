...
module CoopActionSync
  ...
  def self.serialize_actions(turn_num, choices_hash, include_timestamp: true)
    ...
    begin
      ...
      ##MultiplayerDebug.info("COOP-ACTION", "Serialized actions for turn #{turn_num}: #{raw.length} bytes → #{hex.length} hex chars")
  ...
  def self.deserialize_actions(hex_data, from_sid = nil)
    begin
      ...
      # SAFE MARSHAL: Size limit + validation (actions are small, 50KB max)
      max_size = 50_000
