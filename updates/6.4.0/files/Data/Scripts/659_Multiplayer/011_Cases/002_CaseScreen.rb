...
# Actions: [Z] Buy & Open, [A] Open from inventory, [S] Buy to inventory
#===============================================================================
  ...
  class CaseScreen
    ...
    def run
      ...
      loop do
        ...
        unless @animating
          _tick_idle_scroll
          if Input.trigger?(Input::C)            # [Z] Buy & Open
            begin
              _try_buy_and_open
            rescue => e
              @animating = false
          ...
          elsif Input.trigger?(Input::A)          # [A] Open from inventory
            begin
          ...
          elsif _input_s_trigger?                  # [S] Buy to inventory
            begin
    ...
    def _input_s_trigger?
      # S key = key code 83 in some MKXP builds; try letterS constant or raw
      return Input.trigger?(:S) if defined?(Input) && Input.respond_to?(:trigger?) && defined?(:S) rescue false
      # Fallback: check raw key 83
      begin
        return Input.triggerex?(0x53) if Input.respond_to?(:triggerex?)
      rescue; end
      false
    end
    ...
    def _create_info_text
      ...
      bmp.draw_text(0, 26, bmp.width, 24,
        "Cost: #{_case_cost} Plat  |  Owned: #{inv_count}  |  [Z] Buy&Open  [A] Open  [S] Buy  [X] Back", 1)
    ...
    def _draw_action_bar
      ...
      # [S] Buy to inventory
      bmp.font.color = Color.new(0, 0, 0, 100)
      bmp.draw_text(359, 7, 130, 20, "[S] Buy (#{_case_cost})", 0)
      bmp.font.color = can_buy ? Color.new(180, 255, 180, 230) : Color.new(80, 80, 100, 150)
      bmp.draw_text(358, 6, 130, 20, "[S] Buy (#{_case_cost})", 0)
    ...
    # [S] Buy to inventory (no animation)
