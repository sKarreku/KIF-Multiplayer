...
class PokeBattle_SceneEBDX < PokeBattle_Scene
  ...
  def getDefaultEnvironmentData
    ...
    if outdoor && defined?(TerrainEBDX::TALLGRASS) && !data.has_key?("tallGrass")
      ...
      shouldAddGrass = env.nil? || [:None, :Grass, :TallGrass, :MovingWater, :StillWater].include?(env)
      ...
      @battle.pbParty(1).each_with_index do |pkmn, i|
        ...
        if @sprites["battlebg"] && @sprites["battlebg"].respond_to?(:battler)
          ...
          if battlerPos
            ...
            @sprites["pokemon_#{idx}"].y = battlerPos.y
  ...
  def loadUIElements
    ...
    begin
      ...
      @sprites["messageBox"].y = @viewport.rect.height - 106
    ...
    # Message window - positioned at bottom of screen like vanilla
    @sprites["messageWindow"] = Window_AdvancedTextPokemon.newWithSize(
      "", 16, Graphics.height - 96 + 2, Graphics.width - 32, 96, @msgview
    )
  ...
  def forceRefreshAllPositions
    ...
    @battle.battlers.each_with_index do |b, i|
      ...
      sprite.y = battlerPos.y
      sprite.z = battlerPos.z rescue (100 + i)

      # Set constant zoom based on side
      # Player side (i%2 == 0): 1.0, Enemy side (i%2 == 1): 0.65
      baseZoom = (i % 2 == 0) ? 1.0 : 0.65
    ...
    @battle.battlers.each_with_index do |b, i|
      ...
      if shouldUpdatePos
        ...
        if battlerPos
          ...
          @sprites["pokemon_#{i}"].y = battlerPos.y
          @sprites["pokemon_#{i}"].z = battlerPos.z rescue (100 + i)

          # KIF sprite zoom - CONSTANT size regardless of camera zoom
          # Player side (i%2 == 0): 1.0 base (back sprites at normal size)
          # Enemy side (i%2 == 1): 0.65 base (front sprites appear smaller due to distance)
          # Only background zooms for perspective effect, sprites stay same size
          baseZoom = (i % 2 == 0) ? 1.0 : 0.65
  ...
  def playerBattlerSendOut(sendOuts, startBattle = false)
    ...
    playerSendouts.each do |idxBattler, pkmn|
      if @sprites["battlebg"] && @sprites["battlebg"].respond_to?(:battler)
        ...
        if battlerPos
          targetPositions[idxBattler] = { x: battlerPos.x, y: battlerPos.y }
        else
  ...
  def pbAnimation(moveID, user, targets, hitNum = 0)
    ...
    targetIndex = (targets.is_a?(Array) && targets.length > 0) ? targets[0].index : (targets ? targets.index : userIndex)
  ...
  def pbEXPBar(battler, startExp, endExp, tempExp1, tempExp2)
    return if !battler
    @sprites["dataBox_#{battler.index}"].animateEXP(startExp, endExp) if @sprites["dataBox_#{battler.index}"] && @sprites["dataBox_#{battler.index}"].respond_to?(:animateEXP)
    while @sprites["dataBox_#{battler.index}"] && @sprites["dataBox_#{battler.index}"].respond_to?(:animatingEXP) && @sprites["dataBox_#{battler.index}"].animatingEXP
      pbUpdate
    end
  end
