...
module KIFCases
  ...
  class CaseHUD
    ...
    def update
      # Disabled — F7 Cases icon is now rendered by HotkeyHUD
      _set_visible(false)
    end
    ...
    def _hud_briefcase_fill(bmp, ix, iy, c)
      ...
      bmp.fill_rect(ix + 8, iy,      6, 1, c)   # handle top
      bmp.fill_rect(ix + 7, iy + 1,  1, 2, c)   # handle left side
      bmp.fill_rect(ix + 14, iy + 1, 1, 2, c)   # handle right side
      # Body (main rectangle)
      bmp.fill_rect(ix + 1, iy + 3,  20, 16, c)  # body fill
      # Latch bar (horizontal line across middle)
