...
class ChatWindow
  ...
  def draw_messages
    ...
    all_lines[start_index...end_index].each do |line_data|
      ...
      if line_data[:is_owner]
        # Server owner: single gold line
        @messages_sprite.bitmap.font.color = Color.new(255, 200, 0)
        @messages_sprite.bitmap.draw_text(2, y_offset, max_width, line_height, line_data[:line], 0)
      elsif line_data[:is_first]
        # First line of a player message — draw header with title-colored name
        sid       = line_data[:sid].to_s
        name      = line_data[:name].to_s
        sid_part  = "#{sid}/"
        after_name = line_data[:line][(sid_part.length + name.length)..-1] || ""
        td = (MultiplayerClient.title_for(line_data[:sid]) rescue nil)

        x = 2
        # "SIDx/" in dim gray
        @messages_sprite.bitmap.font.color = Color.new(160, 160, 160)
        @messages_sprite.bitmap.draw_text(x, y_offset, max_width, line_height, sid_part, 0)
        sid_w = @messages_sprite.bitmap.text_size(sid_part).width
        x += sid_w

        # Name in title color (or plain gray if no title)
        @messages_sprite.bitmap.font.color = td ? _chat_title_color(td) : Color.new(220, 220, 220)
        @messages_sprite.bitmap.draw_text(x, y_offset, max_width - sid_w, line_height, name, 0)
        name_w = @messages_sprite.bitmap.text_size(name).width
        x += name_w

        # " : message text..." in plain gray
        @messages_sprite.bitmap.font.color = Color.new(220, 220, 220)
        @messages_sprite.bitmap.draw_text(x, y_offset, max_width - sid_w - name_w, line_height, after_name, 0)
      else
        # Continuation wrapped line — plain gray
        @messages_sprite.bitmap.font.color = Color.new(220, 220, 220)
        @messages_sprite.bitmap.draw_text(2, y_offset, max_width, line_height, line_data[:line], 0)
      end
  ...
  def _chat_title_color(td, alpha = 255)
    return Color.new(220, 220, 220, alpha) unless td.is_a?(Hash)
