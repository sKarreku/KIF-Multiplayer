...
TITLE_DEFINITIONS = {
  ...
  # Add new titles here — the client receives full rendering data in packets.
}.freeze
...
def profile_stats_increment!(stats, uuid, stat, amount = 1)
  return unless valid_uuid?(uuid)
  PROFILE_STATS_MUTEX.synchronize do
    stats[uuid] ||= { "wild_fainted"=>0, "wild_captured"=>0, "trainer_battles"=>0, "badges"=>0, "last_updated"=>0 }
    stats[uuid][stat] = (stats[uuid][stat].to_i + amount.to_i).clamp(0, 9_999_999)
...
def profile_stats_set!(stats, uuid, stat, value)
  ...
  PROFILE_STATS_MUTEX.synchronize do
    stats[uuid] ||= { "wild_fainted"=>0, "wild_captured"=>0, "trainer_battles"=>0, "badges"=>0, "last_updated"=>0 }
    stats[uuid][stat] = value.to_i.clamp(0, 9_999_999)
    stats[uuid]["last_updated"] = Time.now.to_i
  end
end
...
def profile_titles_save!(titles)
  PROFILE_STATS_MUTEX.synchronize do
    ...
    titles.each do |uuid, data|
      ...
      safe_titles[uuid] = { "owned" => owned, "active" => active }
    end
...
def profile_titles_load!(titles)
  PROFILE_STATS_MUTEX.synchronize do
    ...
    raw.each do |uuid, data|
      ...
      active = data["active"]
      active = nil unless active.nil? || TITLE_DEFINITIONS.key?(active)
      titles[uuid] = { "owned" => owned, "active" => active }
    end
...
def send_profile_init(c, uuid, sid, clients, profile_titles)
  return unless uuid && valid_uuid?(uuid)
  begin
    data   = profile_titles[uuid] || {}
    owned  = (data["owned"] || []).map { |tid| title_to_hash(tid) }.compact
    safe_send(c, "OWN_TITLES:#{MiniJSON.dump(owned)}")

    active_id = data["active"]
    if active_id && (td_hash = title_to_hash(active_id))
        ...
        if data
              ...
              if client_uuid && !client_uuid.empty? && client_uuid != "NEW"
                ...
                if account
                  ...
                  if account["tid"] == tid
                    ...
                    send_profile_init(c, client_uuid, sid, clients, profile_titles)
                  ...
                  send_profile_init(c, uuid, sid, clients, profile_titles)
          ...
          if data.start_with?("WILD_PLATINUM:")
            begin
              ...
              # Increment wild_fainted stat (server-side event — no extra trust needed)
              profile_stats_increment!(profile_stats, platinum_uuid, "wild_fainted")
            ...
            begin
              ...
              if uuid
                ...
                if last.nil? || (now - last) >= 90.0
                  ...
                  profile_stats_increment!(profile_stats, uuid, "trainer_battles")
                  profile_stats_save!(profile_stats)
            ...
            begin
              ...
              if uuid
                ...
                if wild_captured_timestamps[uuid].length < 12
                  ...
                  profile_stats_increment!(profile_stats, uuid, "wild_captured")
                  profile_stats_save!(profile_stats)
          ...
          if data.start_with?("REQ_PROFILE:")
            begin
              ...
              stats_data = profile_stats[target_uuid] || {
                "wild_fainted"=>0,"wild_captured"=>0,"trainer_battles"=>0,"badges"=>0,"last_updated"=>0
              }

              # Build title data (active title only in response)
              titles_data = profile_titles[target_uuid] || {}
              active_id   = titles_data["active"]
              active_td   = active_id ? title_to_hash(active_id) : nil
              ...
              # Build target name (from platinum_accounts or online client_data)
              target_name = nil
              PLATINUM_MUTEX.synchronize do
                acct = platinum_accounts[target_uuid]
                target_name = acct ? acct["name"] : nil
              end
              target_name ||= client_data.values.find { |cd| cd[:platinum_uuid] == target_uuid }&.dig(:name)
              target_name = sanitize_server_str(target_name.to_s, max_len: 20)
          ...
          if data.start_with?("TITLE_EQUIP:")
            begin
              ...
              if new_active
                td_hash = title_to_hash(new_active)
                msg = "TITLE_UPDATE:#{sid}|#{MiniJSON.dump(td_hash)}"
              else
          ...
          # Sender must connect from an IP in ADMIN_IPS.
          # =============================================
          if data.start_with?("ADMIN_CMD:")
            begin
              peer_ip = c.peeraddr[3] rescue nil
              unless ADMIN_IPS.include?(peer_ip)
                # Silently drop — do not reveal server internals
              ...
              when "give_title"
                ...
                # If target is online, send them their updated titles
