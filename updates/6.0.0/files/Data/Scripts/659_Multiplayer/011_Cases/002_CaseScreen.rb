...
module KIFCases
  ...
  class CaseScreen
    ...
    def initialize
      ...
      @sprites    = {}
      @strip_sprs = []
      @idle_sprs  = []   # continuously scrolling tiles
      @animating  = false
      @winner_sym  = nil
      @winner_tier = nil
    end
    ...
    def _create_info_text
      pool_total = KIFCases.pool_size
      bmp = Bitmap.new(Graphics.width - 40, 50)
      pbSetSystemFont(bmp) if defined?(pbSetSystemFont)
      bmp.font.size  = 14
      bmp.font.bold  = false
      bmp.font.color = Color.new(160, 160, 200, 210)
      bmp.draw_text(0, 0, bmp.width, 24, "Contains #{pool_total} NPT Pokémon across 7 rarity tiers", 1)
      bmp.font.color = Color.new(200, 200, 160, 180)
      bmp.draw_text(0, 26, bmp.width, 24, "Cost: #{CASE_COST} ◆  |  [Z/Enter] Open  |  [X/Esc] Back", 1)
      iy = @strip_y + STRIP_H + 20
      @sprites[:info] = _spr(bmp, 20, iy)
      _create_legend(iy + 55)
    end

    def _create_legend(base_y)
      n       = KIFCases::RARITY_TIERS.size   # 7
      total_w = KIFCases::RARITY_TIERS.sum { |t| t[:weight] }.to_f
      pad     = 10                             # left/right margin
      slot_w  = (Graphics.width - pad * 2) / n
      box_sz  = 10                             # color square size
      bmp_h   = box_sz + 13 + 12              # box + name row + pct row = 35
      ...
      KIFCases::RARITY_TIERS.each_with_index do |tier, i|
        ...
        # Color square — centered horizontally in the slot
        box_x = x + (slot_w - box_sz) / 2
        bmp.fill_rect(box_x,     0,     box_sz, box_sz, Color.new(rgb[0], rgb[1], rgb[2], 230))
        bmp.fill_rect(box_x - 1, -1,    box_sz + 2, 1,  Color.new(0, 0, 0, 100))  # top shadow
        bmp.fill_rect(box_x - 1, box_sz, box_sz + 2, 1, Color.new(0, 0, 0, 100))  # bottom shadow

        # Rarity name — centered below the box
        bmp.font.size  = 11
        bmp.font.color = Color.new(200, 200, 225, 210)
        bmp.draw_text(x, box_sz + 1, slot_w, 13, tier[:name], 1)

        # Percentage — centered below name
        bmp.font.size  = 10
    ...
    def _draw_action_bar
      ...
      bmp.draw_text(w / 2 - 159, 7, 160, 20, "[Z] Open Case (#{CASE_COST} ◆)", 2)
      bmp.font.color = can ? Color.new(255, 215, 0, 230) : Color.new(220, 80, 80, 230)
      bmp.draw_text(w / 2 - 160, 6, 160, 20, "[Z] Open Case (#{CASE_COST} ◆)", 2)
      bmp.fill_rect(w / 2, 5, 2, 20, Color.new(120, 120, 160, 150))
      bmp.font.color = Color.new(0, 0, 0, 100)
      bmp.draw_text(w / 2 + 11, 7, 150, 20, "[X] Back", 0)
      bmp.font.color = Color.new(200, 200, 220, 220)
      bmp.draw_text(w / 2 + 10, 6, 150, 20, "[X] Back", 0)
    ...
    def _init_idle_scroll
      ...
      while x < Graphics.width + TILE_SIZE
        _idle_add_tile(x, pool)
        x += TILE_SIZE
      end
    end

    def _idle_add_tile(x, pool = nil)
      pool ||= KIFCases.build_pool
      tier      = _rand_tier
      tier_pool = pool[tier]
      sp        = tier_pool.empty? ? nil : tier_pool[rand(tier_pool.size)]
      bmp  = _make_tile_bitmap(sp, tier)
    ...
    def _tick_idle_scroll
      ...
      # Remove tiles scrolled fully off the left
    ...
    def _try_open_case
      bal = MultiplayerPlatinum.cached_balance rescue 0
      unless bal >= CASE_COST
        _show_msg("Not enough Platinum!\nYou need #{CASE_COST} ◆ to open a case.")
        return
      end
      ...
      MultiplayerClient.send_data("CASE_OPEN:global")
      ...
      species  = KIFCases.resolve_species(tier, position)
    ...
    # ── Roulette animation ────────────────────────────────────────────────────
    def _run_roulette(winner_tier, winner_position)
      pool = KIFCases.build_pool

      # --- Take ownership of idle sprites so the strip continues seamlessly ---
      idle_data  = @idle_sprs.map { |t| { spr: t[:spr], x: t[:x].to_f } }
      @idle_sprs = []   # idle no longer owns them; _clear_idle_sprites is now a no-op
      n_idle     = idle_data.size

      # Build the roulette tile list (TOTAL_TILES entries, winner at WINNER_IDX)
      roulette_tiles = _build_tile_list(winner_tier, winner_position, pool)

      # Place roulette tiles just to the right of the last idle tile
      right_x = idle_data.last ? idle_data.last[:x] + TILE_STEP : Graphics.width.to_f
      roulette_data = []
      roulette_tiles.each_with_index do |t, i|
        bmp = _make_tile_bitmap(t[:species], t[:tier])
        spr = Sprite.new(@strip_vp)
        spr.bitmap = bmp
        spr.y = (STRIP_H - TILE_SIZE) / 2
      ...
      # Merge: idle first, then roulette (idle tiles exit left, roulette fills in)
      all_data     = idle_data + roulette_data
      @strip_sprs  = all_data.map { |d| d[:spr] }

      # Winner is at n_idle + WINNER_IDX in the combined strip
      actual_winner_idx = n_idle + WINNER_IDX

      # Small random landing sub-pixel offset so the arrow doesn't always stop dead-center
      ...
      # Phase 1 — scroll: fast rush then dramatic slow crawl
      prev_tile_under = ((ptr_x - start_offset) / TILE_STEP).floor
      t2_ease = 0.57   # matches _roulette_ease t2; used for pitch ramp
      ...
      ANIM_FRAMES.times do |frame|
        ...
        # Tick sound whenever a new tile crosses under the arrow
        curr_tile = ((ptr_x - off) / TILE_STEP).floor
        if curr_tile != prev_tile_under
          prev_tile_under = curr_tile
          # Pitch rises from 90 → 130 during the deceleration phase (tension build)
          pitch = if t >= t2_ease
      ...
      # Lock to exact final position
      all_data.each_with_index { |d, i| d[:spr].x = (final_offset + i * TILE_STEP).to_i }

      # Phase 2 — white flash on strip viewport (in/out over 24 frames)
      24.times do |i|
      ...
      # Phase 3 — white highlight border on winner tile
      ...
      # Phase 4 — zoom pulse on winner tile
      ...
      # Brief hold so the player reads the result
      30.times { Graphics.update; Input.update }
      _clear_strip_sprites   # disposes all (repurposed idle + roulette tiles)
    end

    # 3-phase ease with C1 continuity at both transitions:
    #   Phase 1 – abrupt quadratic acceleration  (0  →  2% of time, ~0.14 s)
    #   Phase 2 – constant cruise speed          (2% → 57% of time, ~3.9 s)
    #   Phase 3 – quadratic ease-out deceleration(57%→100% of time, ~3 s)
    # The cruise speed v is derived so that accel/decel slopes match exactly at
    # the boundaries: v = 2 / (t2 - t1 + 1)
    def _roulette_ease(t)
      t1 = 0.02                          # end of acceleration (~0.14 s, very snappy)
      t2 = 0.57                          # start of deceleration (~3 s before end)
      v  = 2.0 / (t2 - t1 + 1.0)        # cruise speed per unit-t (≈ 1.212)
      d1 = v * t1 / 2.0                  # distance at end of accel (≈ 0.030)
      d2 = d1 + v * (t2 - t1)           # distance at start of decel (≈ 0.818)
      if t <= t1
        (v / (2.0 * t1)) * t * t
      elsif t <= t2
        d1 + v * (t - t1)
      else
        s = (t - t2) / (1.0 - t2)       # 0..1 within decel phase
        d2 + (1.0 - d2) * (1.0 - (1.0 - s) ** 2)
      end
    end

    def _build_tile_list(winner_tier, winner_position, pool)
      TOTAL_TILES.times.map do |i|
        if i == WINNER_IDX
          { species: KIFCases.resolve_species(winner_tier, winner_position), tier: winner_tier }
        else
          tier      = _rand_tier
          tier_pool = pool[tier]
          sp        = tier_pool.empty? ? nil : tier_pool[rand(tier_pool.size)]
          { species: sp, tier: tier }
        end
      end
    end
    ...
    def _make_tile_bitmap(species_sym, tier_index)
        ...
        if icon_file
          begin
            ...
            if icon && !icon.disposed?
              # Icons are 2-frame sheets; only the first half is the visible frame
              fw = icon.width / 2
              fh = icon.height
              ix = (TILE_SIZE - fw) / 2
              iy = (TILE_SIZE - fh) / 2
    ...
    def _show_winner_panel
      ...
      rarity_name  = KIFCases.rarity_name(@winner_tier)
      rarity_col   = KIFCases.rarity_color(@winner_tier, 255)
      ...
      panel_bmp.fill_rect(0, 0, pw, 6,  rarity_col)
      panel_bmp.fill_rect(0, ph - 2, pw, 2, rarity_col)
      panel_bmp.fill_rect(0, 0, 2, ph, rarity_col)
      panel_bmp.fill_rect(pw - 2, 0, 2, ph, rarity_col)
      ...
      panel_bmp.draw_text(1, 53, pw, 24, "[ #{rarity_name.upcase} ]", 1)
      panel_bmp.font.color = rarity_col
      panel_bmp.draw_text(0, 52, pw, 24, "[ #{rarity_name.upcase} ]", 1)
      panel_bmp.font.size = 20; panel_bmp.font.bold = true
      panel_bmp.font.color = Color.new(0, 0, 0, 150)
      panel_bmp.draw_text(1, 229, pw, 36, species_name, 1)
      panel_bmp.font.color = Color.new(255, 255, 255, 255)
      panel_bmp.draw_text(0, 228, pw, 36, species_name, 1)
  ...
  class CaseHUD
    ...
    def update
      ...
      begin
        ...
        in_menu   = ($game_temp&.in_menu   rescue false)
        in_battle = ($game_temp&.in_battle rescue false)
    ...
    def _redraw
      ...
      bmp = @spr.bitmap
      pbSetSystemFont(bmp) if defined?(pbSetSystemFont)
      _draw_hud_gem(bmp, 0, 0, DIM_A)
      bmp.font.size = 14; bmp.font.bold = false
      bmp.font.color = Color.new(0, 0, 0, 150)
      bmp.draw_text(28, 4, 44, HUD_H - 4, "F7", 0)
      bmp.font.color = Color.new(255, 215, 0, DIM_A)
      bmp.draw_text(27, 3, 44, HUD_H - 4, "F7", 0)
    end

    def _draw_hud_gem(bmp, ix, iy, alpha)
      c  = Color.new(255, 215,   0, alpha)
      sh = Color.new(180, 100,   0, [alpha - 70, 0].max)
      _hud_gem_fill(bmp, ix + 1, iy + 1, sh)
      _hud_gem_fill(bmp, ix,     iy,     c)
    end

    def _hud_gem_fill(bmp, ix, iy, c)
      bmp.fill_rect(ix + 7,  iy,       8, 2, c)
      bmp.fill_rect(ix + 5,  iy +  2, 12, 3, c)
      bmp.fill_rect(ix + 3,  iy +  5, 16, 5, c)
      bmp.fill_rect(ix + 5,  iy + 10, 12, 3, c)
      bmp.fill_rect(ix + 7,  iy + 13,  8, 2, c)
      bmp.fill_rect(ix + 9,  iy + 15,  4, 3, c)
      bmp.fill_rect(ix + 10, iy + 18,  2, 2, c)
    end
  end
end
