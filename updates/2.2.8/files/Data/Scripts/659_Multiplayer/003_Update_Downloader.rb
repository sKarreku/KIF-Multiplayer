...
module UpdateDownloader
  ...
  def should_use_delta?(manifest)
    # DISABLED: GitHub raw files have LF line endings, local files have CRLF
    # This causes MD5 mismatches. Always use full archive.
    return false
  end
  ...
  def download_full_archive(manifest, version)
    ...
    # Skip MD5 verification for archives - GitHub raw corrupts binary files
    $download_progress = 10.0
    success = HTTPWrapper.http_download_file(archive_url, local_path, nil)
  ...
  def extract_archive(archive_path, dest_dir)
    ...
    MultiplayerDebug.info("UPDATE-DOWNLOADER", "Extracting archive: #{File.basename(archive_path)}") if defined?(MultiplayerDebug)
    MultiplayerDebug.info("UPDATE-DOWNLOADER", "Archive type: #{ext}") if defined?(MultiplayerDebug)
    MultiplayerDebug.info("UPDATE-DOWNLOADER", "Destination: #{dest_dir}") if defined?(MultiplayerDebug)
    ...
    when ".rar"
      ...
      MultiplayerDebug.info("UPDATE-DOWNLOADER", "Executing: #{command}") if defined?(MultiplayerDebug)
      success = system(command)
      if success
        MultiplayerDebug.info("UPDATE-DOWNLOADER", "RAR extraction successful") if defined?(MultiplayerDebug)
      else
        MultiplayerDebug.error("UPDATE-DOWNLOADER", "RAR extraction failed (exit code: #{$?.exitstatus})") if defined?(MultiplayerDebug)
      end
      return success
    when ".7z"
      command = "7z x \"#{archive_path}\" -o\"#{dest_dir}\" -y"
      MultiplayerDebug.info("UPDATE-DOWNLOADER", "Executing: #{command}") if defined?(MultiplayerDebug)
      success = system(command)
      if success
        MultiplayerDebug.info("UPDATE-DOWNLOADER", "7z extraction successful") if defined?(MultiplayerDebug)
      else
        MultiplayerDebug.error("UPDATE-DOWNLOADER", "7z extraction failed (exit code: #{$?.exitstatus})") if defined?(MultiplayerDebug)
      end
      return success
    when ".zip"
      ...
      begin
        require 'zip'
        MultiplayerDebug.info("UPDATE-DOWNLOADER", "Using Ruby Zip library") if defined?(MultiplayerDebug)
        ...
        MultiplayerDebug.info("UPDATE-DOWNLOADER", "ZIP extraction successful") if defined?(MultiplayerDebug)
        return true
      rescue LoadError
        MultiplayerDebug.info("UPDATE-DOWNLOADER", "Ruby Zip unavailable, using 7z") if defined?(MultiplayerDebug)
        command = "7z x \"#{archive_path}\" -o\"#{dest_dir}\" -y"
        MultiplayerDebug.info("UPDATE-DOWNLOADER", "Executing: #{command}") if defined?(MultiplayerDebug)
        success = system(command)
        if success
  ...
  def save_download_state(version, complete, verified)
    # Manual JSON serialization (RGSS has no to_json)
    json = "{\n"
    json += "  \"downloading_version\": \"#{version}\",\n"
    json += "  \"download_complete\": #{complete},\n"
    json += "  \"verified\": #{verified},\n"
    json += "  \"download_started_at\": #{Time.now.to_i}\n"
    json += "}"

    mkdir_p(UPDATES_DIR)
    File.write(STATE_FILE, json)
    MultiplayerDebug.info("UPDATE-DOWNLOADER", "State saved: #{STATE_FILE}") if defined?(MultiplayerDebug)
  rescue => e
