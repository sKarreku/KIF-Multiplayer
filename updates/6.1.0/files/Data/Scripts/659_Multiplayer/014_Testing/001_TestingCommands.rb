...
NPT_TESTING_COMMANDS_ENABLED     = true   # 990_NPT custom species testing
TELEPORT_TESTING_COMMANDS_ENABLED = true  # /teleport map navigation
SPAWN_TESTING_COMMANDS_ENABLED    = true  # /spawn wild battle
  ...
  class << self
    ...
    def parse(text)
      ...
      if SPAWN_TESTING_COMMANDS_ENABLED
        case text
        when /^\/spawn\s+(\w+)(?:\s+(\d+))?$/i
          return { type: :spawn_wild, species_name: $1.strip.upcase, level: ($2 ? $2.to_i : 50) }
  ...
  class << self
    ...
    def send_command(cmd)
      ...
      if SPAWN_TESTING_COMMANDS_ENABLED
        case cmd[:type]
        when :spawn_wild
          return handle_spawn(cmd[:species_name], cmd[:level])
        when :check_species
...
module ChatNetwork
  class << self
    ...
    def handle_spawn(species_name, level)
      species_symbol = species_name.to_sym
      ...
      ChatMessages.add_message("Global", "SYSTEM", "System",
        "Spawning #{sp.real_name} Lv.#{level}...")
