...
module FileIntegrity
  ...
  # Calculate hash for the multiplayer scripts folder (client-side)
  # @param include_npt [Boolean] Also hash 990_NPT when server requires it
  # @return [String] "mp_hash" or "mp_hash|NPT:npt_hash", or "error_..." on failure
  def calculate_client_hash(include_npt: false)
    mp_hash = calculate_hash("Data/Scripts/659_Multiplayer")
    return mp_hash if mp_hash.start_with?("error")

    return mp_hash unless include_npt

    npt_hash = calculate_hash("Data/Scripts/990_NPT")
    return npt_hash if npt_hash.start_with?("error")

    "#{mp_hash}|NPT:#{npt_hash}"
  end
