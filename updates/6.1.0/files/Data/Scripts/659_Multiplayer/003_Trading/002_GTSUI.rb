...
module GTSUI
  ...
  class Scene_GTS
    ...
    def refresh_title
      ...
      # --- Platinum balance badge (right side) ---
      authenticated = defined?(MultiplayerClient) && MultiplayerClient.respond_to?(:platinum_uuid) &&
                      MultiplayerClient.platinum_uuid
      raw_bal  = authenticated ? (MultiplayerPlatinum.cached_balance rescue nil) : nil
      bal_text = raw_bal ? GTSUI.commify(raw_bal) : "?"

      icon_size = 22
      pad       = 6
      gap       = 4
      r         = 5   # corner radius (cross-rect trick)

      # Auto-shrink font if number is very large (e.g. 200,000,000)
      font_size = 18
      b.font.size = font_size
      text_w = b.text_size(bal_text).width
      max_text_w = 180
      if text_w > max_text_w
        font_size = [(18 * max_text_w.to_f / text_w).to_i, 11].max
        b.font.size = font_size
        text_w = b.text_size(bal_text).width
      end

      badge_w = pad + icon_size + gap + text_w + pad
      badge_h = 30
      badge_x = b.width - badge_w - 6
      badge_y = (TITLE_H - badge_h) / 2

      # Rounded rectangle background (two overlapping rects form the pill shape)
      bg = Color.new(0, 0, 0, 190)
      b.fill_rect(badge_x + r, badge_y,         badge_w - 2*r, badge_h,     bg)  # horizontal bar
      b.fill_rect(badge_x,     badge_y + r,     badge_w,       badge_h-2*r, bg)  # vertical bar

      # Platinum icon
      icon_x = badge_x + pad
      icon_y = badge_y + (badge_h - icon_size) / 2
      begin
        icon_bmp = RPG::Cache.load_bitmap("Graphics/Pictures/Trainer Card/", "platinum_icon")
        b.stretch_blt(Rect.new(icon_x, icon_y, icon_size, icon_size),
                      icon_bmp, Rect.new(0, 0, icon_bmp.width, icon_bmp.height))
      rescue
        b.font.size = 13
        b.font.color = Color.new(255, 210, 0)
        b.draw_text(icon_x, icon_y, icon_size, icon_size, "Pt", 1)
      end

      # Balance number (vertically centered in badge)
      b.font.size  = font_size
      b.font.color = WHITE
      b.draw_text(badge_x + pad + icon_size + gap, badge_y, text_w + 2, badge_h, bal_text)
    end
    ...
    begin
        ...
        # Refresh title whenever platinum balance changes (covers buy + server-pushed PLATINUM_BAL)
        if defined?(MultiplayerPlatinum)
          current_bal = MultiplayerPlatinum.cached_balance
    ...
    def apply_local_after_buy_ok(listing)
      # NOTE: Item/Pokémon delivery is already handled by GTS_EXECUTE (gts_apply_execution in
      # 003_Client.rb). Do NOT add them here — doing so would cause a duplication bug where the
      # buyer receives two copies (one from GTS_EXECUTE, one from this GTS_BUY OK handler).
      # This method now only triggers an autosave to persist changes made by GTS_EXECUTE.
      begin
        GTSUI.autosave_safely
      rescue => e
