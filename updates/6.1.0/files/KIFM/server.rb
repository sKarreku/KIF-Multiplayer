...
module FileIntegrity
  ...
  # Returns [mp_hash, npt_hash_or_nil]
  # mp_hash:  hash of 659_Multiplayer alone
  # npt_hash: hash of 990_NPT alone, or nil if folder not present
  def calculate_server_hash
    ...
    mp_hash  = calculate_hash(File.join(base_path, "659_Multiplayer"))
    npt_path = File.join(base_path, "990_NPT")
    npt_hash = Dir.exist?(npt_path) ? calculate_hash(npt_path) : nil
...
# ===========================================
@server_mp_hash, @server_npt_hash = FileIntegrity.calculate_server_hash
@server_npt_check = !@server_npt_hash.nil?
if @server_mp_hash.start_with?("error")
  puts "[WARNING] Could not calculate file integrity hash: #{@server_mp_hash}"
  MultiplayerDebug.error("INTEGRITY", "Hash calculation failed: #{@server_mp_hash}")
  @server_mp_hash = nil
  @server_npt_check = false
else
  npt_note = @server_npt_check ? " + 990_NPT (#{@server_npt_hash[0..7]}...)" : ""
  puts "[Server] File integrity check enabled — 659_Multiplayer: #{@server_mp_hash[0..7]}...#{npt_note}"
  MultiplayerDebug.info("INTEGRITY", "MP hash: #{@server_mp_hash} | NPT hash: #{@server_npt_hash.inspect}")
end
  ...
  if IO.select([server], nil, nil, 0.01)
    begin
      ...
      if @server_mp_hash
        ...
        safe_send(client, "ASSIGN_ID:#{session_id}|VERIFY:#{@server_mp_hash}#{npt_flag}")
      else
        ...
        if data
          ...
          if data.start_with?("FILE_VERIFY:")
            begin
              ...
              if @server_mp_hash.nil?
                safe_send(c, "INTEGRITY_OK")
                MultiplayerDebug.info("INTEGRITY", "#{sid} verification skipped (server hash unavailable)")
                next
              end

              # Parse client payload: "mp_hash" or "mp_hash|NPT:npt_hash"
              parts          = payload.split("|")
              client_mp_hash = parts[0].to_s.strip
              client_npt_hash = (parts.find { |p| p.start_with?("NPT:") } || "").sub("NPT:", "").strip

              # Compare per folder and send a specific failure reason
              fail_reason = nil
              if client_mp_hash != @server_mp_hash
                fail_reason = "Your multiplayer scripts (659_Multiplayer) don't match the server. Please update KIF Multiplayer."
                MultiplayerDebug.error("INTEGRITY", "#{sid} 659_Multiplayer mismatch — client: #{client_mp_hash[0..7]}... server: #{@server_mp_hash[0..7]}...")
              elsif @server_npt_check && client_npt_hash != @server_npt_hash.to_s
                fail_reason = "Your custom Pokemon files (990_NPT) don't match the server. Please update your NPT files."
                MultiplayerDebug.error("INTEGRITY", "#{sid} 990_NPT mismatch — client: #{client_npt_hash[0..7]}... server: #{@server_npt_hash[0..7]}...")
              end
              ...
              if fail_reason
                ...
                safe_send(c, "INTEGRITY_OK")
                MultiplayerDebug.info("INTEGRITY", "#{sid} file integrity verified (MP: #{client_mp_hash[0..7]}...)")
              end
            rescue => e
          ...
          # =========================================
          # === CASES: Open a case (server-authoritative)
          # =========================================
          if data.start_with?("CASE_OPEN:")
            begin
              ...
              unless uuid
                ...
                MultiplayerDebug.warn("CASES", "#{sid} CASE_OPEN without platinum auth")
                next
              end

              # Rarity weights — MUST match KIFCases::RARITY_TIERS weights in client
              case_weights = [40, 25, 15, 10, 6, 3, 1]
              case_cost    = 200
              success      = false
              new_balance  = 0
              tier         = 0
              ...
              PLATINUM_MUTEX.synchronize do
                ...
                if account
                  ...
                  if current >= case_cost
                    account["platinum"]            = current - case_cost
                    account["platinum_spent_cases"] = account["platinum_spent_cases"].to_i + case_cost
                    new_balance                     = account["platinum"]
                    success                         = true
                    ...
                    roll  = rand(total)
                    case_weights.each_with_index do |w, i|
                      roll -= w
                      if roll < 0
                        tier = i
                        break
                    ...
                    # Random position within the tier (client mods by tier size)
              ...
              if success
                PLATINUM_MUTEX.synchronize { server_bank["balance"] += case_cost }
                platinum_save!(platinum_accounts)
                server_bank_save!(server_bank)
                safe_send(c, "CASE_RESULT:#{tier}|#{position}|#{new_balance}")
                MultiplayerDebug.info("CASES", "#{sid} opened case → tier #{tier}, pos #{position}, new balance #{new_balance} Pt, server bank #{server_bank["balance"]} Pt")
              else
                safe_send(c, "CASE_ERROR:INSUFFICIENT")
                MultiplayerDebug.warn("CASES", "#{sid} insufficient balance for case (cost #{case_cost} Pt)")
              end
            rescue => e
              MultiplayerDebug.error("CASES", "CASE_OPEN error for #{sid}: #{e.message}")
