        ...
        while (line = @socket.gets)
          ...
          if data.start_with?("ASSIGN_ID:")
            ...
            if parts.length > 1 && parts[1].start_with?("VERIFY:")
              ...
              # Calculate client's integrity hash (include 990_NPT if server requires it)
              ...
              # No integrity check required - request event state immediately
          ...
          if data.start_with?("INTEGRITY_OK")
            ...
            # Request event state sync after successful authentication
            if defined?(EventSystem)
              send_data("REQ_EVENTS")
