...
class PokemonBattlerSprite < RPG::Sprite
  ...
  def create_family_outline
    ...
    begin
      ...
      # Include fusion info in cache key to prevent stale outlines when Pokemon is fused
      fusion_key = ""
      if @pkmn.respond_to?(:isFusion?) && @pkmn.isFusion?
        # Include fused partner species if available for unique cache key
        fusion_key = "_fused"
      ...
      # Pre-generate animation frames (cache by effect + colors + mask size + species/fusion)
      anim_cache_key = "#{cache_key}_#{effect}_#{colors.join('_')}_#{final_w}_#{final_h}"
