        ...
        if data
          ...
          if data == "PING" || data.start_with?("PING:")
            if data.start_with?("PING:")
              ...
              # Store this client's ping in client_data (for broadcasting)
              if client_data[c]
                # Calculate approximate ping (server perspective is less accurate)
                # The client will calculate the actual RTT
                client_data[c][:last_ping_time] = Time.now
              end
