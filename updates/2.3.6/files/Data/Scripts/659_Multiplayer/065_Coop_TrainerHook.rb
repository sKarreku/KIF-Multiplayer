...
def pbTrainerBattle(
  ...
  if in_squad && current_map_id && current_event_id
    ...
    if sync_wait &&
      ...
      if MultiplayerClient.is_battle_cancelled?(sync_wait[:battle_id])
        ...
        trainer = nil
        begin
          trainer_raw = BinHex.decode(sync_wait[:trainer_hex])
          trainer = SafeMarshal.load(trainer_raw, max_size: 1_000_000)
          SafeMarshal.validate(trainer)
        rescue => e
  ...
  if in_squad
    ...
    trainer_raw = Marshal.dump(trainer)
    trainer_hex = BinHex.encode(trainer_raw) rescue ""
...
def start_coop_trainer_battle_as_joiner(sync_wait, trainer_id, trainer_name, end_speech, can_lose, outcome_var)
  ...
  # Unmarshal trainer data from initiator (Remix mode: use initiator's trainer)
  ...
  trainer_raw = BinHex.decode(trainer_hex) rescue nil
  if trainer_raw.nil?
    ##MultiplayerDebug.error("COOP-TRAINER-BATTLE", "Failed to decode trainer hex data!")
    MultiplayerClient.clear_trainer_sync_wait
    return 2 # Loss
  end
  ...
  begin
    ...
    ##MultiplayerDebug.error("COOP-TRAINER-BATTLE", "Failed to unmarshal trainer data: #{e.message}")
    MultiplayerClient.clear_trainer_sync_wait
    return 2 # Loss
  end
