...
class PokemonSummary_Scene
  ...
  def update_family_name_animation(effect, colors)
    # Check if family fonts are enabled
    return unless $PokemonSystem.familyfont == 1
  ...
  def create_summary_family_name
    ...
    # Check if family fonts are enabled
    if defined?($PokemonSystem) && $PokemonSystem && $PokemonSystem.familyfont == 0
      return  # Family fonts disabled
    end
  ...
  def drawPageOne
    ...
    # Hide original name + Pokeball if has family AND fonts are enabled
    if @pokemon && @pokemon.has_family? && $PokemonSystem.familyfont == 1
