  ...
  class << self
    ...
    def press?(button)
      if defined?(CouchCoopInput) && defined?(CouchCoopConfig) && CouchCoopConfig.enabled?
        ...
        # Keyboard-only mode
        if mode == "keyboard"
          # When unfocused, use Win32API for keyboard
          return CouchCoopInput.input_pressed_via_keyboard?(button) unless is_focused

          # When focused, let MKXP-Z handle keyboard normally
          return couch_coop_original_press?(button)

        # Controller-only mode
        elsif mode == "controller"
          # When focused, block ALL keyboard inputs
          if is_focused
            # Check if ANY key mapped to this button is pressed on keyboard
            if CouchCoopInput.input_pressed_via_keyboard?(button)
              return false  # Block keyboard input entirely
            end
          end
          # Let controller input through from MKXP-Z
          return couch_coop_original_press?(button)
        end
      end
    ...
    def dir4
      if defined?(CouchCoopInput) && defined?(CouchCoopConfig) && CouchCoopConfig.enabled?
        ...
        if mode == "keyboard"
          ...
          # Block keyboard directional input in controller-only mode when focused
          if is_focused
            # Check if any directional key is pressed via keyboard
            if CouchCoopInput.input_pressed_via_keyboard?(Input::UP) ||
              ...
              return 0  # Block keyboard directional input
            end
          end
          return couch_coop_original_dir4
        end
