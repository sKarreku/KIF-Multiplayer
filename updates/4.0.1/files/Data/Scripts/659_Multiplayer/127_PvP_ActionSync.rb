  ...
  def self.serialize_choice(choice, battler)
    ...
    when :UseMove
      ...
      if defined?(MultiplayerDebug)
        MultiplayerDebug.info("PVP-ACTION", "Serializing move: target #{target_index} -> mirrored #{mirrored_target}")
      end

      serializable_choice = [:UseMove, move_index, nil, mirrored_target]
